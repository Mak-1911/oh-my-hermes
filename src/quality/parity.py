...
PARITY_CAPABILITIES: tuple[ParityCapability, ...] = (
    ...
    ParityCapability(
        ...
        omh_surface="`ultrawork` coordinated lanes, `omh runtime team-readiness`, runtime handoff payloads, worker-protocol guidance, wrapper sessions, and runtime observations.",
        status="available",
        evidence=(
            "skills/ulw-work/SKILL.md",
    ...
    ParityCapability(
        ...
        status="available",
        evidence=("src/surfaces/hud.py", "src/plugin_bundle/omh/tools/hud_tool.py", "src/wrapper/sessions.py", "src/runtime/artifacts.py"),
    ...
    ParityCapability(
        ...
        omh_surface="`omh setup --with-mcp --mcp-host ...`, `omh mcp manifest`, `omh mcp config-recipe`, `omh mcp serve`, `omh probe`, and MCP preference/host-config/runtime-call separation.",
        status="available",
        evidence=("src/mcp/bridge.py", "src/mcp/host_config.py", "src/commands/mcp.py", "src/maintenance/probe.py", "tests/test_mcp_bridge.py"),
        missing_piece="OMH can write supported local host MCP config entries, but it does not independently inspect live host sessions; the host or wrapper must record load/session evidence.",
        v1_decision="Ship a dependency-free stdio bridge with only allowlisted local status, recommendation, and probe tools.",
        user_value="Operators can connect OMH to MCP-capable hosts through setup or copy-paste host recipes without exposing arbitrary shell or hidden execution.",
    ...
    ParityCapability(
        ...
        omh_surface="`loop`, `ultrawork`, `ralplan`, loop queue ticks, verification tiers, and failure-mode status cards.",
        status="available",
        evidence=("src/workflows/goal_loop.py", "src/commands/loop.py", "skills/ulw-loop/SKILL.md", "skills/ulw-work/SKILL.md"),
    ...
    ParityCapability(
        ...
        omh_surface="`omh setup`, `omh doctor`, `omh update`, `omh uninstall`, `omh release checklist`, `omh release product-readiness`, `omh release hermes-smoke`, and `omh demo hermes-ux-quality`.",
        status="available",
        evidence=("src/commands/setup.py", "src/maintenance/doctor.py", "src/maintenance/release.py", "install.sh", "install.ps1"),
