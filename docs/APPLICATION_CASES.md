...
| G2 | GitHub PR/Issue event operations | `workflow_skill` | Installed `github-event-ops` skill or chat route | `github-event-ops` / `github-event-ops` | A GitHub event card is not webhook delivery, GitHub API mutation, label application, review completion, CI rerun, or fix execution evidence. |
| G3 | Multi-agent Kanban board | `workflow_skill` | Installed `agent-board` skill or chat route | `agent-board` / `agent-board` | Hermes can render an `agent_board` card; a board card is not proof that another Hermes target accepted, worked, heartbeat-ed, or completed unless target-specific evidence exists. |
| G4 | Memory and skill curation review | `workflow_skill` | Installed `memory-sync` skill or chat route | `memory-sync` / `memory-sync` | Hermes can render a `memory_curation` card; a curation review is not Hermes internal memory, `MEMORY.md`, `USER.md`, or skill-file modification evidence. |
| G5 | Gateway-native intent card | `workflow_skill` | Installed `gateway-intent-card` skill or chat route | `gateway-intent-card` / `gateway-intent-card` | Hermes can render a `gateway_intent` card; a gateway intent card is not platform login, message send, thread mutation, attachment upload, or delivery evidence. |
| G6 | Executor runtime readiness | `workflow_skill` | Installed `executor-runtime-readiness` skill or chat route | `executor-runtime-readiness` / `executor-runtime-readiness` | Runtime readiness is not executor dispatch, plugin load, tool invocation, code execution, review, CI, or merge evidence. |
...
| G8 | Voice and mobile operator | `workflow_skill` | Installed route for voice/mobile normalization before concrete workflow selection | `voice-operator` / `voice-operator` | Hermes can render a `voice_operator` card; a voice operator card is not speech recognition proof, mobile notification delivery, platform action, or accepted execution evidence. |
| G9 | MCP and external toolbelt readiness | `workflow_skill` | Installed `toolbelt-readiness` skill or chat route | `toolbelt-readiness` / `toolbelt-readiness` | Hermes can render a `toolbelt_readiness` card; a toolbelt card is not MCP install, credential validation, API access, connector invocation, or successful workflow execution evidence. |
| G10 | Ops command-board and service quality card | `workflow_skill` | Installed `ops-observability-card` skill or chat route | `ops-observability-card` / `ops-observability-card` | Hermes can render an `ops_observability` card and provider-neutral service-quality board; an observability card is not billing truth, provider quota truth, live metric-provider access, complete tracing, SLO pass, incident closure, remediation completion, performance proof, or workflow completion evidence. |
...
generated from `src/quality/grounded_score.py`; when scenarios change, this
table should change with them.

| Scenario | User message tested | Chat route | Playbook | Coding handoff behavior | Score |
| --- | --- | --- | --- | --- | --- |
| Startup SaaS product triage | `결제 실패 이슈가 자주 나와` | `feedback-triage` / `feedback_triage` | `feedback-triage` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Startup SaaS product triage with strategy follow-up | `결제 실패 피드백을 모아서 회의 주제와 다음 전략을 정리해줘` | `feedback-triage` / `feedback_triage` | `feedback-triage` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Open-source issue-to-PR preparation | `이 이슈 PR로 만들 수 있게 정리해줘` | `ralplan` / `plan` | `safe-feature-change` | Prepared handoff only; execution, verification, review, CI, and merge stay unobserved until runtime evidence exists. | `10/10` |
| AI agent product QA | `쿠버네티스 장애 상황에서 Cloudy가 적절히 진단하나?` | `ultraqa` / `qa_review` | `release-readiness-review` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Dangerous refactor plan-first routing | `이거 위험한 리팩터링 같아` | `ralplan` / `plan` | `safe-feature-change` | Prepared handoff only; execution, verification, review, CI, and merge stay unobserved until runtime evidence exists. | `10/10` |
| AI coding safety audit | `AI가 했다고 했는데 실제로 뭐 했는지 모르겠다` | `code-review` / `review_check` | `release-readiness-review` | Prepared handoff only; execution, verification, review, CI, and merge stay unobserved until runtime evidence exists. | `10/10` |
| Product feature shaping | `온보딩을 더 부드럽게 만들고 싶어` | `deep-interview` / `clarification` | `deep-interview-to-plan` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Release gate review | `릴리즈 전에 README claim이 실제 코드와 맞는가, doctor/harness가 통과하는가 봐줘` | `code-review` / `review_check` | `release-readiness-review` | Prepared handoff only; execution, verification, review, CI, and merge stay unobserved until runtime evidence exists. | `10/10` |
| Korean release readiness status | `릴리즈 준비 상태 점검해줘` | `code-review` / `review_check` | `release-readiness-review` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Repeated refactor workflow | `레거시 서비스를 위험 분석, 변경 범위 제한, 테스트 전략, Codex 구현, 리뷰, 회귀 테스트 순서로 리팩터링하고 싶어` | `ai-slop-cleaner` / `plan` | `safe-feature-change` | Prepared handoff only; execution, verification, review, CI, and merge stay unobserved until runtime evidence exists. | `10/10` |
| Personal multi-agent work hub | `지금은 Hermes가 답할 차례인지, coding handoff를 준비할 차례인지, review gate를 열 차례인지 정리해줘` | `plan` / `plan` | `local-pipeline-buildout` | Prepared handoff only; execution, verification, review, CI, and merge stay unobserved until runtime evidence exists. | `10/10` |
| Consulting or agency operating template | `고객사 프로젝트별 요구사항 정리, 조사, 구현 handoff, QA, 리뷰, 릴리즈 보고 운영 템플릿이 필요해` | `plan` / `plan` | `local-pipeline-buildout` | Prepared handoff only; execution, verification, review, CI, and merge stay unobserved until runtime evidence exists. | `10/10` |
| Operating rhythm history | `회의록 히스토리 관리하고 스크럼 스프린트 회고 운영 리듬 정리해줘` | `operating-rhythm` / `operating_rhythm` | `operating-rhythm-history` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Leadership report package | `create a PPT report package for a monthly leadership status deck` | `report-package` / `report_package` | `report-package` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Materials processing package | `엑셀 매출 리포트를 PDF로 만들고 렌더 QA까지 준비해줘` | `materials-package` / `materials_package` | `materials-processing` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Korean meeting notes to presentation material | `이 회의록을 발표자료로 만들어줘` | `materials-package` / `materials_package` | `materials-processing` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Reliability incident review | `run an incident postmortem SLO error budget service reliability review` | `reliability-review` / `reliability_review` | `reliability-incident-review` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Idea-to-deploy product loop | `take this product idea from plan to deploy and monitor safely` | `idea-to-deploy` / `app_delivery_loop` | `idea-to-deploy` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| CTO loop | `run a CTO loop for roadmap architecture tradeoffs delivery risk and release readiness` | `cto-loop` / `cto_loop` | `cto-loop` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Deploy and monitor | `deploy and monitor this release with rollback and health checks` | `deploy-and-monitor` / `deploy_monitor_plan` | `deploy-and-monitor` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| English product shaping | `I need to improve our onboarding but I don't know where to start` | `deep-interview` / `clarification` | `deep-interview-to-plan` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Workflow learning improvement | `I want Hermes to learn from this workflow and improve the skill next time` | `workflow-learning` / `workflow_learning` | `workflow-learning` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Korean workflow learning improvement | `이 워크플로우 다음엔 더 잘하게 개선해줘` | `workflow-learning` / `workflow_learning` | `workflow-learning` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Korean task improvement memory | `다음부터 이 작업 더 잘하게 기억해줘` | `workflow-learning` / `workflow_learning` | `workflow-learning` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Korean answer-to-skill improvement | `이 답변 다음엔 더 잘하게 스킬 고쳐줘` | `workflow-learning` / `workflow_learning` | `workflow-learning` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
...
| Korean research to strategy report | `조사해서 전략 보고서로 만들어줘` | `strategy-brief` / `strategy_brief` | `research-to-strategy-brief` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| GitHub event ops delivery | `Make this GitHub issue into a PR, run review, update docs, and tell me what changed` | `github-event-ops` / `github_event_ops` | `github-event-ops` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Korean GitHub issue labeling and PR prep | `새 이슈 들어오면 라벨링하고 PR 준비해줘` | `github-event-ops` / `github_event_ops` | `github-event-ops` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Executor runtime selection | `Should I use Codex or Claude Code for this coding task?` | `executor-runtime-readiness` / `executor_runtime_readiness` | `executor-runtime-readiness` | Readiness card only; executor handoff, dispatch, execution, verification, review, CI, and merge stay unobserved until separate runtime evidence exists. | `10/10` |
| Korean Codex open session | `codex 세션 켜서 작업 시작하게 해줘` | `executor-runtime-readiness` / `executor_runtime_readiness` | `executor-runtime-readiness` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
...
| Coding-agent progress status | `Codex 작업이 진행중인지 확인하고 지금 어떤 상태인지 알려줘` | `ultrawork` / `handoff` | `ultrawork` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Korean PR merge readiness status | `지금 PR 머지 준비 됐는지 알려줘` | `ultrawork` / `handoff` | `ultrawork` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Korean Codex progress status | `코덱스가 지금 어디까지 했는지 알려줘` | `ultrawork` / `handoff` | `ultrawork` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Korean Codex current-activity status resolves the owner-selection surface | `코덱스가 지금 뭐하고있는지 알려줘` | `executor-runtime-readiness` / `executor_runtime_readiness` | `ultrawork` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
...
| Korean Hermes memory review | `Hermes가 기억하는 내용 한번 점검하자` | `memory-sync` / `memory_curation` | `memory-sync` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
...
| Korean loop cost and latency status | `루프 비용이랑 지연시간 상태 보여줘` | `ops-observability-card` / `ops_observability` | `ops-observability-card` | No executor handoff by default; Hermes keeps the workflow retained until observed evidence changes. | `10/10` |
| Loopability-gated goal cycle | `./loop make this project a 10k star OSS` | `loop` / `loop` | `Direct skill invocation` | Direct skill invocation; no observed runtime work is claimed. | `10/10` |
| Direct legacy ultraprocess invocation follows the delivery alias | `$ultraprocess research the repo, plan, implement, code-review, sync docs, and prepare a PR` | `ultrawork` / `handoff` | `Direct skill invocation` | Prepared handoff only; execution, verification, review, CI, and merge stay unobserved until runtime evidence exists. | `10/10` |
