...
"""
    ...
    def test_release_product_readiness_cli_summarizes_product_story(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("56/56 negative-control cases", gates["routing_precision"]["summary"])
            self.assertIn("129/129 interventions", gates["routing_precision"]["summary"])
    ...
    def test_release_evidence_bundle_cli_packages_optional_write_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "Routing precision: 56/56 negative controls, 129/129 interventions "
            ...
            self.assertEqual(payload["summary"]["routing_precision_passing"], 56)
            self.assertEqual(payload["summary"]["routing_precision_total"], 56)
            self.assertEqual(payload["summary"]["routing_precision_overroute_count"], 0)
            self.assertEqual(payload["summary"]["routing_precision_intervention_passing"], 129)
            self.assertEqual(payload["summary"]["routing_precision_intervention_total"], 129)
