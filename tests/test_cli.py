...
"""
    ...
    def test_release_product_readiness_cli_summarizes_product_story(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("180/180 route hints aligned", gates["route_hint_alignment"]["summary"])
            ...
            self.assertIn("55/55 negative-control cases", gates["routing_precision"]["summary"])
            self.assertIn("127/127 interventions", gates["routing_precision"]["summary"])
            ...
            self.assertIn("90/90 common request cases", gates["common_request_coverage"]["summary"])
            ...
            self.assertIn("common requests 90/90", gates["hermes_ux_quality"]["summary"])
    ...
    def test_release_evidence_bundle_cli_packages_optional_write_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("Chat card coverage: 60/60 (generic ack 0)", stdout)
            self.assertIn("Context brief coverage: 10/10 (route hints 9, catalog hints 1)", stdout)
            self.assertIn(
                "Routing precision: 55/55 negative controls, 127/127 interventions "
            ...
            self.assertEqual(payload["summary"]["chat_card_coverage_passing"], 60)
            self.assertEqual(payload["summary"]["chat_card_generic_ack_count"], 0)
            self.assertEqual(payload["summary"]["route_hint_alignment_aligned"], 180)
            ...
            self.assertEqual(payload["summary"]["routing_precision_passing"], 55)
            self.assertEqual(payload["summary"]["routing_precision_total"], 55)
            self.assertEqual(payload["summary"]["routing_precision_overroute_count"], 0)
            self.assertEqual(payload["summary"]["routing_precision_intervention_passing"], 127)
            self.assertEqual(payload["summary"]["routing_precision_intervention_total"], 127)
            ...
            self.assertEqual(payload["summary"]["common_request_coverage_passing"], 90)
            self.assertEqual(payload["summary"]["common_request_coverage_total"], 90)
    ...
    def test_chat_route_hint_summary_renders_operator_readable_hint(self) -> None:
        ...
        self.assertIn("[omh] omh-img-summary looks relevant.", stdout)
        ...
        self.assertIn("- Open omh-img-summary - enabled", stdout)
    ...
    def test_demo_chat_card_coverage_keeps_user_facing_routes_dedicated(self) -> None:
        ...
        self.assertEqual(payload["summary"]["case_count"], 60)
        self.assertEqual(payload["summary"]["passing_count"], 60)
    ...
    def test_demo_chat_card_coverage_summary_is_human_readable(self) -> None:
        ...
        self.assertIn("Result: 60/60 workflow cards dedicated (all passing)", stdout)
    ...
    def test_demo_route_hint_alignment_keeps_router_and_awareness_hints_in_sync(self) -> None:
        ...
        self.assertEqual(payload["summary"]["case_count"], 180)
        self.assertEqual(payload["summary"]["hinted_count"], 180)
        self.assertEqual(payload["summary"]["aligned_count"], 180)
        ...
        self.assertEqual(explicit_payload["summary"]["case_count"], 180)
    ...
    def test_demo_route_hint_alignment_summary_is_human_readable(self) -> None:
        ...
        self.assertIn("Result: 180/180 route hints aligned (all passing)", stdout)
        self.assertIn("Hints present: 180/180; missing hints: 0; mismatches: 0", stdout)
        self.assertIn(
