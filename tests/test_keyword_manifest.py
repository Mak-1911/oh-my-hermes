...
class KeywordManifestTests(unittest.TestCase):
    ...
    def test_keyword_manifest_includes_guard_rules_and_skill_triggers(self) -> None:
        ...
        self.assertIn("ultrawork", skills)
        self.assertIn("$ultrawork", skills["ultrawork"]["triggers"])
        self.assertEqual(skills["ops-observability-card"]["exposure"], "workflow_skill")
        self.assertTrue(skills["ops-observability-card"]["install_visibility"])
        self.assertFalse(skills["ops-observability-card"]["compatibility_alias"])
