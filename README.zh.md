...
OMH 将 **92 个**可安装的 workflow skill 组织为6个容易理解的能力族。

其中 10 个是 workflow engine - `deep-interview`, `loop`, `ralph`, `ralplan`, `team`, `ultragoal`, `ultraprocess`, `ultraqa`, `ultrawork`, `web-research` - 它们渲染为 `ulw-` 标签，
只看状态行就能知道正在运行哪一类 skill。其余 82 个使用 `omh-`。
